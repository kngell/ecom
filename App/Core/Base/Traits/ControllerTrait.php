...
trait ControllerTrait
{
    ...
    public function registerSubscribedServices(array $otherServices = []): void
    {
        ...
        $fileServices = YamlFile::get('events');
    ...
    {
        $columns = Application::diGet($columnString);
        if ($columns) {
            return array_filter($columns->columns(), fn ($column) => isset($column[$part]) && $column[$part] === true ? $column['dt_row'] : []);
        }
    }
    ...
    {
        if (is_array($controllers = YamlFile::get('controller'))) {
    ...
    public function dispatchEvent(string $event, string $method, array $context, object $controller)
    {
        ...
        if (isset($this->eventDispatcher)) {
            $this->eventDispatcher->dispatch(new $event($method, $context, $controller), $event::NAME);
        }
    }
            ...
            foreach ($args as $arg) {
                foreach ($arg as $property => $class) {
                    if (class_exists($class)) {
                        $output = ($property === 'dataColumns' || $property === 'column') ? $this->$property = $class : $this->$property = Application::diGet($class);
                    }
                }
            }
            return $output;
    ...
    {
        $_params = [];
        $extendedEvents = YamlFile::get('extend_events');
    ...
    private function resolveListeners(array $parameters): void
    {
        foreach ($parameters as $listeners => $values) {
            if (isset($listeners)) {
                ...
                $listenerObject = Application::diGet($values['class']);
    ...
    private function resolveSubscribers(array $parameters): void
    {
        foreach ($parameters as $subscribers => $values) {
            if (isset($subscribers)) {
                $subscriberObject = Application::diGet($values['class']);
